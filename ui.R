                    ...
                    dashboardBody(
                     ...
                     fluidRow(
                       box(
                         ...
                         p("Two methods can be used to import Parquet data:"),
                                             tags$ul(
                                               tags$li("Load the data in RAM - slower to load but faster operations. Better if large RAM available."),
                                               tags$li("Access data directly on-disk from Parquet file - fast loading but slower operations. Better for computers with limited RAM.")
                                             ),
                                             "Choose one of the two methods:",br(),
                                             selectInput("loadingMethod", label = "Data loading method",
                                                         choices = list("Load into RAM" = 1, "Keep on-disk" = 2),
                                                         selected = 1),
